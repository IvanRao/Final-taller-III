...
function crearJuego() {
  $('#saveJuego').addEventListener('click', function() {
    getBase64($('#fileJuegos').files[0]).then(b64 => {
      const juegos = {
        nombre: $('#nombre').value,
        precio: $('#precio').value,
...
function Editar(id) {
  ...
  RestApi.get('/api/juegos/' + $('#id_juego').value).then(juegos => {
...
function editarProducto() {
  $('#editarJuego').addEventListener('click', function() {
    ...
    if ($('#fileJuegosEditar').files[0] == null) {
      ...
      getBase64($('#fileJuegosEditar').files[0]).then(b64 => {
        const juegos = {
          id: $('#id_juego').value,
          nombre: $('#editar_nombre').value,
          precio: $('#editar_precio').value,
...
function abrirPanel() {
  const tbody = $('#juegos');
  RestApi.get('/api/juegos').then(juegos => {
    $_removeChilds(tbody);
    juegos.forEach(prod => {
...
function cargarSelectConsolas() {
  ...
  RestApi.get('/api/consolas').then(consolas => {
    consolas.forEach(c => {
